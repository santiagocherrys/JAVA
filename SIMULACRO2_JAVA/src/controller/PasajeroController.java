...
public class PasajeroController {
    ...
    public static void findPasajeroByName(){
        PasajeroModel objPasajeroModel = new PasajeroModel();
        String input = JOptionPane.showInputDialog("Ingrese el nombre del pasajero a buscar sin apellido, no necesariamente tiene que ser completo, puede arrojar multiples registros si coinciden los nombres\n");

        List<Pasajero> listaPasajeros = objPasajeroModel.findPasajeroByName(input);
        if(listaPasajeros.isEmpty()){
            JOptionPane.showMessageDialog(null,"No se encuentra pasajero con ese nombre");
        }else{
            String imprimir = "Estos son las coincidencias \n";

            for(Pasajero pasajero: listaPasajeros){
                imprimir += pasajero.toString() + "\n";
            }

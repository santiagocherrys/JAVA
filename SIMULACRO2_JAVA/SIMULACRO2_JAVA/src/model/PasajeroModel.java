...
public class PasajeroModel implements CRUD {
    ...
    public List<Pasajero> findPasajeroByName(String pasajero){
        List<Object> listaPasajeros = findAll();
        List<Pasajero> listPasajero = new ArrayList<>();

        //Revisar si hay pasajeros
        ...
        if(listaPasajeros.size()> 0){
            ...
            try{
                //Escribirmos el query en SQl
                String sql = "SELECT id_pasajero, nombre, apellido, documento_identidad FROM pasajero WHERE nombre LIKE(?);";

                PreparedStatement objPrepare = objConnection.prepareStatement(sql);

                objPrepare.setString(1,"%" + pasajero + "%");
                //Ejecutar el query y obtener el resultado
